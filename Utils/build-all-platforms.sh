...
if [ ${targets[0]} -eq 1 ]; then
	# x86
	required bash ./scripts/build-plugin.sh --type "$type" --platform windows --arch x86 --makesys '\"Visual Studio 16 2019\" -A Win32' --version $version_plugin $rebuild_option $wrapper_only
	# x64
	required bash ./scripts/build-plugin.sh --type "$type" --platform windows --arch x86_64 --makesys '\"Visual Studio 16 2019\" -A x64' --version $version_plugin $rebuild_option $wrapper_only
fi
